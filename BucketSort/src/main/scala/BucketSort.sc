...
def iterationConditionLET: (a,b) = if (a<=b) true else false

def iterationConditionLT: (a,b) = if (a<b) true else false

def iterationConditionHET: (a,b) = if (a>=b) true else false

def iterationConditionHT: (a,b) = if (a>b) true else false
 ...
 *              integers equal or smaller than <b>pivot</b>
 */
def lowerSplitHO(list: List[Int], pivot: Int, iterationCondition: (a,b) => Boolean): List[Int] = {
  list match{
    case Nil => Nil
    case h :: t =>
      if (iterationCondition(h,pivot)) h::lowerSplit(t,pivot) else lowerSplit(t,pivot)
  }
}
 ...
 *              integers equal or greater than <b>pivot</b>
 */
def higherSplitHO(list: List[Int], pivot: Int,iterationCondition: (a,b) => Boolean): List[Int] = {
  list match{
    case Nil => Nil
    case h :: t =>
      if (iterationCondition(h,pivot)) h::higherSplit(t,pivot) else higherSplit(t,pivot)
  }
}
...
def quickSortHO(list: List[Int]) : List[Int] = {
  list match {
    ...
    case h :: Nil => h :: Nil
    case h :: t =>
      quickSort(lowerSplitHO(t,h, iterationConditionLET)) ::: h :: quickSort(higherSplitHO(t,h, iterationConditionHT))
  }
}
...
@tailrec
def max(list : List[Int], a : Int, iterationCondition : (a, b) => Boolean): Int={

  list match
    case Nil => throw new Exception("Empty List doesn't have any element to compare")

    case h :: Nil => if(iterationCondition(h,a)) h else a

    case h :: t => if(iterationCondition(h,a)) max(t, h)else max(t,a)
 ...
 */
@tailrec
def min(list : List[Int], a : Int, iterationCondition : (a, b) => Boolean): Int={
  list match
    case Nil => throw new Exception("Empty List doesn't have any element to compare")

    case h :: Nil => if(iterationCondition(h,a)) h else a

    case h :: t => if(iterationCondition(h,a)) min(t, h) else min(t,a)
}
 ...
 * @return <b>Int</b>, This is the approximation of <b>a</b>.
 */
def approachHO(a : Double, iterationCondition : (a, b) => Boolean): Int={

  if(iterationCondition(a,a.toInt+0.5)) a.toInt else a.toInt + 1
...
def width(min : Int, max : Int, k : Int): Int={
  ...
  approachHO((max-min)/sqrt(k.toDouble), iterationConditionLT)
 ...
 */
def intervalsQuantity(n : Int): Int={
  approach(sqrt(n.toDouble), iterationConditionLT)
